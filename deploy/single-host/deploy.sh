...
: "${SWAP_SIZE_GB:=8}"; : "${REDAMON_SKIP_RAM_GATE:=false}"; : "${REDAMON_BUILD_PARALLEL:=}"; : "${DOCKER_DNS:=}"
...
build_deploy_env() {
  ...
  {
    echo "# generated by deploy.sh -- do not edit; transient, chmod 600, rm'd on cleanup"
    for k in APP_DIR REPO_URL REPO_BRANCH REMOTE_USER HOST_IP SSH_PORT \
             ...
             SWAP_SIZE_GB REDAMON_SKIP_RAM_GATE REDAMON_BUILD_PARALLEL DOCKER_DNS \
